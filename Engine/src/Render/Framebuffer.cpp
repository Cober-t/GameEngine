...
SDL_GPUTextureFormat ToSDLFramebufferFormat(FramebufferTextureFormat format)
{
    switch (format)
    {
...
{
    GraphicsDevice::Get().BeginFramebufferRenderPass(this);
}

void Framebuffer::Unbind()
{
    GraphicsDevice::Get().EndActiveRenderPass();
}
...
void Framebuffer::Invalidate()
{
    ...
    for (const auto& spec : m_ColorAttachmentSpecifications)
    {
        ...
        LOG_CORE_ASSERT(sampler, "SDL_CreateGPUSampler(framebuffer) failed: {0}", SDL_GetError());
    ...
    if (m_DepthAttachmentSpecification.TextureFormat != FramebufferTextureFormat::None)
    {
        ...
        LOG_CORE_ASSERT(m_DepthAttachment, "SDL_CreateGPUTexture(depth) failed: {0}", SDL_GetError());
    }

    // Create readback buffer for pixel picking
...
int Framebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y)
{
    ...
    texRegion.texture = m_ColorAttachments[attachmentIndex];
    texRegion.mip_level = 0;
    texRegion.layer = 0;
    texRegion.x = (uint32_t)x;
    texRegion.y = (uint32_t)y;
    texRegion.z = 0;
    texRegion.w = 1;
    texRegion.h = 1;
    texRegion.d = 1;
    ...
    SDL_DownloadFromGPUTexture(copyPass, &texRegion, &transferInfo);
...
void Framebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
{
    ...
    texTransferInfo.pixels_per_row = m_Specification.Width;
    texTransferInfo.rows_per_layer = m_Specification.Height;
    ...
    texRegion.y = 0;
    texRegion.z = 0;
    texRegion.w = m_Specification.Width;
    texRegion.h = m_Specification.Height;
    texRegion.d = 1;
...
uintptr_t Framebuffer::GetColorAttachmentRenderID(uint32_t index) const
{
...
SDL_GPUTexture* Framebuffer::GetColorAttachmentTexture(uint32_t index) const
{
    LOG_CORE_ASSERT(index < m_ColorAttachments.size(), "Attachment index out of bounds");
