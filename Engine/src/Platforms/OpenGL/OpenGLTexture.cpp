...
namespace Cober {
		...
		: _width(width), _height(height)
	{
		...
		glTextureStorage2D(m_RendererID, 1, _internalFormat, _width, _height);
		...
		LOG_CORE_TRACE("Render Texture success");
	}
	

	OpenGLTexture::~OpenGLTexture()
	{
		LOG_CORE_TRACE("Render Texture cleaned up");
	...
	void OpenGLTexture::SetData(void* data, uint32_t size)
	{
		...
		glTextureSubImage2D(m_RendererID, 0, 0, 0, _width, _height, _dataFormat, GL_UNSIGNED_BYTE, data);
	}
